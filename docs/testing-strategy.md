...
- Playwright E2E smoke tests protect browser-level catalog loading, registration, login validation, cart checkout, My Orders visibility, and order detail status rendering.
- Optional full-stack Playwright tests protect the same customer journey against the real local gateway and Docker runtime when `QUICKBITE_FULLSTACK_E2E=1` is set.
...
Run the optional full-stack customer journey after starting Docker Compose:
...
The full-stack Playwright suite is documented for local execution and can be added to CI once the runner budget allows starting SQL Server, Kafka, APIs, gateway, and the frontend together.
